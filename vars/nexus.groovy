def nexus(COMPONENT) {
  ...
  command = "curl -f -v -u admin:admin123 --upload-file ${FILENAME} http://172.31.11.10:8081/repository/${COMPONENT}/${FILENAME}"
...
def make_artifacts(APP_TYPE, COMPONENT) {
  ...
  def FILENAME=COMPONENT+'-'+get_branch_exec+'.zip'
  if(APP_TYPE == "NGINX") {
    command = " zip -r ${FILENAME} node_modules dist config build src todo.conf"
